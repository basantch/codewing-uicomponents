...
const ResponsiveControl = ({device, onChange}) => {
  return <ResponsiveButtons className="cw__responsive-buttons">
      <button 
...
const ControlGroup = (Component) => ({
  ...
  const handleOnChange = (_value) => {
    onChange(responsive ? {...value, [responsive?.currentState]: _value} : _value)
  }
    
  const border = divider ? ` cw__divider-${divider}` : '';
  return (
    <ControlItemStyles className={`cw__control-item ${direction || ''}${border}`}>
      {
        label && <header>
              <label>
                {label}
                {help && <Tooltip title={help} trigger="click">
                  <ControlHelpStyle>{Icons.help}</ControlHelpStyle>
                </Tooltip>}
              </label>
              {
                ((_ref.current !== value) || responsive) && <div className="cw__action-buttons">
                  {
                    _ref.current !== value && <button tabIndex={0} className="cw__reset-button" onClick={() => onChange(_ref.current)}>Reset</button>
                  }
                  {
                    responsive && <ResponsiveControl onChange={responsive.setState} device={responsive.currentState} />
                  }
                </div>
              }
            </header>
      }
      {description && direction!=="horizontal" && <div className="cw__control-description">{description}</div>}
      <section className={className || ''}>
        <Component value={responsive ? value[responsive?.currentState] : value} onChange={(res) => handleOnChange(res)} {...rest}/>
      </section>
      {description && direction==="horizontal" && <div className="cw__control-description" style={{margin: "16px 0 0"}}>{description}</div>}
    </ControlItemStyles>
  );
};
