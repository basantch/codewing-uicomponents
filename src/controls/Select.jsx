...
const Icons = {
    close: <svg width="9" height="10" viewBox="0 0 9 10" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M8.12428 1.46449L1.05321 8.53556M1.05321 1.46449L8.12428 8.53556" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>    
}
...
const SelectedBadge = ({text, onCancel}) => {
    return <SelectedBadgeStyle className="cw__selected-badge">
            {text}
            <button type="button" aria-label="cancel" className="cw__cancel" onClick={onCancel}>
                {Icons.close}
            </button>
        </SelectedBadgeStyle>
}
...
const Select = ({
    ...
    const selectedValue = options?.find(a => a.value === value)?.label || "";

    useEffect(() => {
        if(isMultiple){
            setSelectOptions(value.length > 0 ? removeItems(options, value) : options)
        }
    }, [value])

    const handleSelectOnKeyDown = (_value) => (e) => {
        if(e.type === "click" || (e.type === "keydown" && e.key === "Enter")){
            onChange(isMultiple ? (!value.includes(_value) ? [...value, _value] : value.filter(v => v != _value)) : _value)
    ...
    return <OutsideClickHandler onOutsideClick={() => setOpen(false)}>
        <SelectStyles className={`cw__custom-select${open && ' open' || ''}${isMultiple ? ' is-multiple' : ''}`}>
            <div 
                tabIndex={0}
            ...
            >
                {
                    isMultiple && value?.map(val => {
                        const _selectedValue = options.find(a => a.value === val)?.label;
                        return <SelectedBadge key={val} text={_selectedValue} onCancel={() => onChange(value.filter(a => a !== val))} />
                    })
                }
                {
                    isMultiple && value.length <= 0 && <span className="placeholder">{placeholder || ''}</span>
                }
                <input 
                    tabIndex={-1}
                    type="text" 
            ...
            {selectOptions.length > 0 && open && <div className="cw__select-dropdown">
                <ul className="cw__select-options">
                    {selectOptions?.map(({value: _value, label})=>{
