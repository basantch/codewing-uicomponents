...
const SelectButton = ({
    ...
    return <Tooltip title={title}>
        <SelectButtonStyles 
            tabIndex={0} 
            className={
                `cw__select-button${checked && ' cw__select-button-checked' || ''}`
            } 
            style={style}
        ...
        >
            <span className="cw__select-button">
                <input tabIndex={-1} className="cw__select-button-input" type="checkbox" value={value} checked={checked} onChange={onChange} {..._rest} />
            </span>
...
const SelectButtonGroup = ({
    ...
    const handleOnChange = (_value) => () => {
        if(multiple){
            onChange(!value.includes(_value) ? [...value, _value] : value.filter(a => a != _value));
        }else{
            onChange(_value)
        }
    }

    return <SelectButtonGroupStyles className={`cw__select-button-group ${className || ''} ${separate?'cw__separate':''} ${size || ''}`}>
        {
