...
const ColorPalette = ({className, colors, ...rest}) => {
    return <ColorPaletteSwatches tabIndex={0} className={`cw__color-palette-swatches ${className}`} {...rest}>
        <div className="cw__color-palette-swatches-inner">
            {
                colors?.map(color => {
                    return <span key={color.name} className="cw__color-palette-swatch" style={{backgroundColor: color.color}} />
                })
            }
        </div>
...
const ColorPalettes = ({value, onChange, colorPalettes, ...ControlGroup}) => {
        ...
        <>
            ...
            />
            {
                isVisible && <Popover className="cw__color-palettes-popover">
                    <OutsideClickHandler onOutsideClick={() => setIsVisible(false)}>
                        <ColorPaletteOptions className="cw__color-palette-options">
                            {
                                colorPalettes && colorPalettes.map(palette => {
                                    return <div key={palette.name} className="cw__color-palette-option">
